    ...
    <div class="max-w-6xl mx-auto py-8">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <x-form back="{{ route('user_management_view') }}"
                action="{{ route('user_management_add_update_post', $user->id) }}" method="post" judul="Edit User" deskripsi="{{ $user->name }} ({{ $user->email }})">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="tel" name="no_telp" id="no_telp" value="{{ old('no_telp', $user->no_telp) }}"
                            class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2.5 px-4"
                    ...
                    <div>
                        ...
                        <select name="role" id="role" required
                            ...
                            <option value="pembeli" {{ old('role', $user->role) == 'pembeli' ? 'selected' : '' }}>Pembeli
                            </option>
                        </select>
                        @error('role')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
